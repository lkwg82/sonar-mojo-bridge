...
public class Maven3SonarEmbedderTest {
    ...
    public void shouldDetectMavenfromRunningInstance() throws MavenEmbedderException {
        if (System.getenv("_") == null) {
            throw new IllegalStateException("not running from maven");
        }
        try {
            Maven3SonarEmbedder.configure().usePomFile("pom.xml").goal(goal).build();
    ...
    @Test
    public void testSettingWrongAlternativeMavenHomeAsDirectory() throws MavenEmbedderException {
        try {
            Maven3SonarEmbedder.configure()
                    .logLevel(Logger.LEVEL_WARN)
                    .usePomFile("pom.xml")
                    .showErrors(true)
                    .setAlternativeMavenHome(new File("."))
                    .goal("versions:help")
                    .build()
                    .run();
        } catch (MavenEmbedderException e) {
            assertThat(e).isExactlyInstanceOf(MavenEmbedderException.class);
        }
    }
}
